...
resource "aws_iam_role" "lambda" {
  name               = format("%s-draining-function-role", substr(var.autoscaling_group_name, 1, 32))
...
resource "aws_iam_role_policy" "lambda_execution_policy" {
  name = format("%s-draining-function-policy", substr(var.autoscaling_group_name, 1, 32))
...
resource "aws_iam_role" "lifecycle" {
  name               = format("%s-lifecycle-role", substr(var.autoscaling_group_name, 1, 32))
